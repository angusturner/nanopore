...
def download_ecoli_data():
    ...
    reads_url = "https://zenodo.org/records/7995806/files/bonito_local_basecalled.fastq.gz?download=1"
    reads_path = Path("data/raw/ecoli_nanopore_reads.fastq.gz")
    
    success = download_file(
        reads_url, 
        reads_path,
        "E. coli nanopore reads (FASTQ format with quality scores)"
    )
...
def download_reference_genome():
    ...
    success = download_file(
        ref_url,
        ref_path,
        "E. coli reference genome (K-12 MG1655)"
    )
...
def verify_downloads():
    """Check that downloaded files exist and look reasonable"""
    print("\n🔍 Verifying downloads...")
    
    files_to_check = [
        ("data/raw/ecoli_nanopore_reads.fasta.gz", "Nanopore reads"),
        ("data/references/ecoli_reference.fasta", "Reference genome")
    ]
...
2. Start interactive notebook:
   uv run jupyter lab notebooks/

3. Explore your data in the results/ directory
...
- Each read represents a single DNA molecule that was sequenced
            ...
            print("\n❌ Some downloads may have failed. Check the errors above.")
            sys.exit(1)
    else:
