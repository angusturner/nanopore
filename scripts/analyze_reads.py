...
def load_reads(fastq_file):
    ...
    for i, record in enumerate(SeqIO.parse(handle, file_format)):
        ...
        if file_format == "fastq" and hasattr(record, 'letter_annotations'):
            if 'phred_quality' in record.letter_annotations:
                ...
                read_data['mean_quality'] = sum(record.letter_annotations['phred_quality']) / len(record.letter_annotations['phred_quality'])
    ...
    if has_quality:
        print(f"📊 Quality scores available: ✅ Yes! (FASTQ format)")
    else:
        print(f"📊 Quality scores available: ❌ No (FASTA format)")
...
def analyze_sequence_composition(reads_data, sample_size=10000):
    ...
    print(f"🔬 Analyzing sequence composition (sampling {min(sample_size, len(reads_data)):,} reads)...")
    ...
    composition = {nuc: count / total_nucleotides * 100 
...
def create_visualizations(stats, composition, homopolymer_data, reads_data, output_dir="results"):
    ...
    plt.axvline(stats['n50'], color='red', linestyle='--', 
                label=f'N50: {stats["n50"]:,.0f} bp')
    plt.axvline(stats['mean_length'], color='orange', linestyle='--',
    ...
    plt.axhline(stats['total_bases'] / 2, color='red', linestyle='--',
                label='50% of bases')
    ...
    plt.axvline(composition['mean_gc_content'], color='red', linestyle='--',
...
def print_summary(stats, composition, homopolymer_data):
    ...
    for base in ['A', 'T', 'G', 'C']:
        ...
        if data['count'] > 0:
            print(f"  {base}: {data['count']:,} runs (max: {data['max_length']}bp, avg: {data['mean_length']:.1f}bp)")
    
    print(f"\n💡 ML ENGINEERING INSIGHTS:")
    print(f"  - Variable sequence lengths: {stats['min_length']}-{stats['max_length']} bp")
    print(f"  - This is like having variable-length text documents")
    print(f"  - GC bias: {abs(50 - gc_pct):.1f}% deviation from expected 50%")
    print(f"  - Homopolymer errors are the main ML challenge for nanopore data")
    print(f"  - Long reads enable genome assembly but have higher error rates")
